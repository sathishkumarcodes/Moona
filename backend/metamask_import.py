"""
MetaMask and Phantom wallet integration for importing cryptocurrency holdings
Connects to wallets via frontend and reads wallet balances
"""
...
metamask_router = APIRouter(prefix="/metamask", tags=["metamask"])
phantom_router = APIRouter(prefix="/phantom", tags=["phantom"])

class MetaMaskConnection(BaseModel):
    connected: bool
...
class MetaMaskWalletData(BaseModel):
    address: str
...
async def get_token_price(symbol: str, chain: str = "ethereum") -> float:
...
@metamask_router.post("/connect")
async def connect_metamask(
    wallet_data: MetaMaskWalletData,
    current_user: UserData = Depends(get_current_user_dependency)
):
    """Import holdings from MetaMask wallet"""
    try:
        ...
        for balance_data in wallet_data.balances:
            try:
                ...
                # Get current price
                price = await get_token_price(symbol, chain)
        ...
        logger.error(f"Error importing from MetaMask: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to import from MetaMask: {str(e)}")

@metamask_router.get("/status")
async def get_metamask_status(
    current_user: UserData = Depends(get_current_user_dependency)
):
    ...
    try:
        ...
        logger.error(f"Error getting MetaMask status: {str(e)}")
        return MetaMaskConnection(
            connected=False,
            holdings_count=0,
            error=str(e)
...
@metamask_router.delete("/disconnect")
async def disconnect_metamask(
    current_user: UserData = Depends(get_current_user_dependency)
):
    """Remove all MetaMask holdings (disconnect wallet)"""
    try:
        if current_user.id == "mock_user_123" or len(current_user.id) < 32:
            raise HTTPException(status_code=401, detail="Please log in")
        ...
        return {"success": True, "message": "MetaMask holdings removed"}
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error disconnecting MetaMask: {str(e)}")
...
async def connect_phantom(
    ...
    try:
        ...
        # Import holdings (same logic as MetaMask)
        imported_count = 0
        failed_count = 0
        import_errors = []
